...
void fixMusicPointers()
{
	...
	for (int i = 0; i < 256; i++)
	{
		...
		else
		{
			...
			if (checkPos > 0x10000)
			{
				std::cerr << musics[i].name << ": Echo buffer exceeded total space in ARAM by 0x" << hex4 << checkPos - 0x10000 << " bytes. Will break on accurate emulators." << std::dec << std::endl;
				std::cout << musics[i].name << " exceeds Echo buffer. Will break on accurate emulators." << std::endl;
				//quit(1);
			}
